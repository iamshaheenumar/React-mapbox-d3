(function(global, factory) {
  ...
  function SortedDirectAccessDoublyLinkedList() {
    this._valueAccessor = function(obj) { // accessor to the value to sort on
      return obj.value;
    };
    this._idAccessor = function(obj) { // accessor to the uniq id
      return obj.id;
    };
  ...
  SortedDirectAccessDoublyLinkedList.prototype.add = function(datum) {
    ...
    if (this.size === 0) {
      //special case: no node in the list yet
      this._min = this._max = this._closestTo0 = dln;
    } else {
      if (dln.value <= this._min.value) {
        //special case: new datum is the min
        dln.next = this._min;
        dln.next.prev = dln;
        this._min = dln;
      } else if (dln.value >= this._max.value) {
        //special case: new datum is the max
        dln.prev = this._max;
        dln.prev.next = dln;
        this._max = dln;
      } else {
  ...
  SortedDirectAccessDoublyLinkedList.prototype.remove = function(datum) {
    ...
    if (this.size === 1) {
      //special case: last item to remove
      ...
      if (dln === this._min) {
        //special case: datum is the min
        this._min = this._min.next;
        this._min.prev = null;
      } else if (dln === this._max) {
        //special case: datum is the max
  ...
  function beeswarm() {
    ...
    var distributeOn = function(datum) { // accessor to the x value
      return datum.x;
    };
    ...
    _beeswarm.side = function(_) {
      ...
      if (_ === "symetric" || _ === "positive" || _ === "negative") {
        side = _;
      }
    ...
    function initArrangement() {
      ...
      xBasedDataManager = new SortedDirectAccessDoublyLinkedList()
        ...
        .addMany(arrangement);
      xBasedColliderManager = new SortedDirectAccessDoublyLinkedList().valueAccessor(
        function(d) {
          return d.fixed;
        }
      );
      yBasedColliderManager = new SortedDirectAccessDoublyLinkedList().valueAccessor(
        function(d) {
          return d.free;
        }
      );
    ...
    function findNearestPossibleCollider(dln, visitedDln, direction) {
      if (visitedDln === null) {
        ...
        // visitedDln is close enought
        if (isFinite(visitedDln.datum.free)) {
          // visitedDln is already arranged, and hence is the nearest possible x-based collider
          return visitedDln.datum;
        }
        // continue finding
        return findNearestPossibleCollider(
          dln,
          visitedDln[direction],
          direction
        );
      }
    }

    function visitToGatherXBasedPossibleColliders(
      dln,
      visitedDln,
      direction,
      ...
      if (visitedDln === null) {
        ...
        // continue gathering
        visitToGatherXBasedPossibleColliders(
          dln,
          visitedDln[direction],
          direction,
          xBasedPossibleColliders
    ...
    function gatherXBasedPossibleColliders(datum) {
      ...
      var nearestXPrevAlreadyArrangedData = findNearestPossibleCollider(
        dln,
        dln.prev,
        "prev"
      );
      var nearestXNextAlreadyArrangedData = findNearestPossibleCollider(
        dln,
        dln.next,
        "next"
      );
    ...
    function yPosRelativeToXbpc(xbpc, d) {
      ...
      return (
        Math.sqrt(
          minSquareDistanceBetweenCircles - Math.pow(d.fixed - xbpc.fixed, 2)
        ) + 1e-6
      );
    }
    ...
    function areCirclesColliding(d0, d1) {
      ...
      return (
        Math.pow(d1.free - d0.free, 2) + Math.pow(d1.fixed - d0.fixed, 2) <
        minSquareDistanceBetweenCircles
      );
    }

    function visitToDetectCollisionWithOther(
      datum,
      visitedDln,
      direction,
      visitCount
      ...
      if (visitedDln === null) {
        // special case: y_max reached, no collision detected
        return false;
      } else if (
        direction === "prev"
        ...
        // continue visit
        return visitToDetectCollisionWithOther(
          datum,
          visitedDln[direction],
          direction,
          visitCount++
    ...
    function collidesWithOther(datum, visitedDln) {
      ...
      if (
        visitToDetectCollisionWithOther(datum, visitedDln, "prev", visitCount++)
      ) {
        return true;
      } else {
